...
from  pyqed.floquet.floquet_add_all_zak_phase import TightBinding, FloquetBloch
import numpy as np
import matplotlib.pyplot as plt
import time
import os
...
def test_Gomez_Leon_2013(E0=200, number_of_step_in_b=51, nt=21, slice_b_value=None):
    """
    ...
    # 1. Parameters Setup
    omega = 10
    E_over_omega = np.linspace(0, E0/omega, 101) # Reduced steps for faster testing
    ...
    c_grid = np.linspace(0.01,0.09,9)
    ...
    for c in c_grid:
        for b_idx, b in enumerate(b_grid):
            ...
            data_path = f"/Volumes/Shuoyi's SSD/sum_over_all_bands/chiral_k={num_k:.1f}_nt_5_new/c={c:.4f}/floquet_data_Gomez_Leon_test_b={b:.2f}/"
            ...
            results = floquet_model._run(k_vals)
            # energies, states = floquet_model.run(k_vals)
            
            # --- Calculate Winding Number (Vectorized) ---
            winding_number_grid[b_idx] = floquet_model.cumulative_winding_number()
        ...
        output_filename = os.path.join(f"/Volumes/Shuoyi's SSD/sum_over_all_bands/chiral_k={num_k:.1f}_nt_5_new/c={c:.4f}/", f"winding_c_{c:.5f}.npy")
        np.save(output_filename, winding_number_grid)
        ...
        plt.tight_layout()
        save_dir = f"/Volumes/Shuoyi's SSD/sum_over_all_bands/chiral_k=200.0_new/c={c:.4f}_floquet_winding_map_0_2.png"
        ...
        # plt.title(f'Floquet Winding Number Map (Band {winding_number_band})')
        # plt.tight_layout()
        # save_dir = f'MacBook_local_data/sum_over_all_bands/chiral_k={num_k:.1f}_nt_5_new/c={c:.4f}_floquet_winding_map.png'
        # abs_save_dir = f'MacBook_local_data/sum_over_all_bands/chiral_k={num_k:.1f}_nt_5_new/absolute_band_gap_map_c={c:.4f}_.png'
        # rel_save_dir = f'MacBook_local_data/sum_over_all_bands/chiral_k={num_k:.1f}_nt_5_new/arelative_band_gap_map_c={c:.4f}.png'
        ...
        #     slice_save_dir = 'MacBook_local_data/sum_over_all_bands/chiral/'
        ...
        #     slice_save_dir = 'MacBook_local_data/sum_over_all_bands/chiral_k=1000.0_nt_5_new/'
        #     os.makedirs(slice_save_dir, exist_ok=True)
...
def test_1D_2norbs(E0 = 0.5, number_of_step_in_omega = 11, nt = 101):
    ...
    omega = np.linspace(0.1,0.05,6)
    omega = np.linspace(0.025,0.03,11)
    ...
    for omg_idx, omg in enumerate(omega):
        ...
        data_path=f"/Volumes/Shuoyi's SSD/E_omega_b=0.7_realistic_parameter_E_max_0.5_sum_over_all_bands_sin/floquet_data_1D_2norbs_test_omega={omg:.5f}/"
        floquet_model = tb_model.Floquet(omegad=omg, E0=E, nt=nt, polarization=[1], data_path=data_path)
        _ = floquet_model._run(k_vals)

        winding_number_grid[omg_idx]=floquet_model.cumulative_winding_number()
        # --- Save Results ---
        output_filename = os.path.join(data_path, f"winding_omega_{omg:.5f}.npy")
        np.save(output_filename, winding_number_grid[omg_idx])
    ...
    plt.title(f'Floquet Winding Number Map (Band {winding_number_band}/)')
    plt.tight_layout()
    save_dir = f"/Volumes/Shuoyi's SSD/E_omega_b=0.7_realistic_parameter_E_max_0.5_sum_over_all_bands_sin/floquet_winding_map.png"
    os.makedirs(os.path.dirname(save_dir), exist_ok=True)
    plt.savefig(save_dir, dpi=300)
    plt.show()
...
def test_Gomez_Leon_2013_test_only(E0=20, number_of_step_in_b=11, nt=11, slice_b_value=None):
    ...
    for External_E in E:
        ...
        for b_idx, b in enumerate(b_grid):
            os.makedirs(f'MacBook_local_data/no_reassign_test_new_test_only/chiral_k={num_k:.1f}_nt_5_new/c=0/floquet_data_Gomez_Leon_test_b={b:.2f}/overlap_matrix', exist_ok=True)
            os.makedirs(f'MacBook_local_data/no_reassign_test_new_test_only/chiral_k={num_k:.1f}_nt_5_new/c=0/floquet_data_Gomez_Leon_test_b={b:.2f}/overlap_matrix_valance_band', exist_ok=True)
            os.makedirs(f'MacBook_local_data/no_reassign_test_new_test_only/chiral_k={num_k:.1f}_nt_5_new/c=0/floquet_data_Gomez_Leon_test_b={b:.2f}/overlap_matrix_conduction_band', exist_ok=True)
            print(f"--- Starting Calculation for b = {b:.4f} ---")
            # Create tight-binding model
            coords = [[0, 0], [b, 0]]
            # coords = [[0, 0], [b, c]]
            ...
            # Initialize Floquet model
            data_path = f'MacBook_local_data/no_reassign_test_new_test_only/chiral_k={num_k:.1f}_nt_5_new/c=0/floquet_data_Gomez_Leon_test_b={b:.2f}/'
            floquet_model = tb_model.Floquet(omegad=omega, E0=E, nt=nt, polarization=[1,1j], data_path=data_path)
...
if __name__ == "__main__":
    ...
    # test_Gomez_Leon_2013_realistic_parameter()
    # test_Gomez_Leon_2013_test_only()
    test_1D_2norbs()