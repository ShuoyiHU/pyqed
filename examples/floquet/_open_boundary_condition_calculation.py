...
def main(E_min = 0, E_max=5, E_step =201, omega=0.5, OBC_cells=20, nt = 101):
    ...
    base_data_path = f"MacBook_local_data/open_boundary_condition_calculation/coords_{model.coords[0][0]}_{model.coords[1][0]}_t1_{model.relative_Hopping[0]}_t2_{model.relative_Hopping[1]}/floquet_data_Gomez_Leon_test_b={b:.2f}/E_max_{E_max}_omega_{omega}_OBC_cells_{OBC_cells}_nt_{nt}"
...
def main_quasi_2D(E_min=0, E_max=5, E_step=201, omega=0.5, OBC_cells=20, nt=101, chirality=1.0):
    """
    Main function to perform a sweep over a list of E-field values,
    analyze the Floquet system for each, and save all plots and data
    into organized subdirectories. This version is modified to respond to
    chiral/elliptical light.

    Args:
        E_min (float): Starting electric field strength.
        E_max (float): Maximum electric field strength.
        E_step (int): Number of E-field steps to simulate.
        omega (float): Driving frequency of the light field.
        OBC_cells (int): Number of unit cells in the 1D chain.
        nt (int): Number of Floquet sidebands to include in the calculation.
        chirality (float): Defines the light polarization.
                           0: Linear polarization along the x-axis.
                           1: Left-circularly polarized (LCP) light.
                          -1: Right-circularly polarized (RCP) light.
                           Values between -1 and 1 correspond to elliptical polarization.
    """
    print("--- Starting Floquet OBC Analysis for Multiple E-Fields ---")
    print(f"Polarization Chirality = {chirality}")

    # --- 1. System and Path Parameters ---
    N0 = nt // 2
    all_p = np.arange(-N0, N0 + 1)
    E_field_list = np.linspace(E_min, E_max, E_step)
    print(f"Will analyze the system for E-fields from {E_min} to {E_max}\n")

    # --- 2. Define the SSH Model (once) ---
    model = TightBinding(
        coords=[[0, 0], [0.75, 0.1]],
        relative_Hopping=[1.5, 1],
        lattice_constant=[1.0, 0]
    )
    norbs = model.norb

    # MODIFICATION: Add chirality to the path to distinguish simulation results
    base_data_path = (f"MacBook_local_data/open_boundary_condition_calculation/"
                      f"coords_{model.coords[0][0]}_{model.coords[1][0]}_t1_{model.relative_Hopping[0]}_t2_{model.relative_Hopping[1]}/"
                      f"floquet_data_chiral/chirality_{chirality}_E_max_{E_max}_omega_{omega}_OBC_cells_{OBC_cells}_nt_{nt}")
    ...
    for E_field in E_field_list:
        ...
        # --- MODIFICATION FOR CHIRAL LIGHT ---
        # The vector potential for elliptically polarized light is A(t) = (E/w) * (cos(wt), eta*sin(wt)), where eta is the chirality.
        # This leads to a modification of the hopping t_ij -> t_ij * exp(i * A(t) . d_ij).
        # The p-th Fourier component of the exponential term is J_p(Z_mag) * exp(-i*p*phi),
        # where Z_mag = sqrt(Z_x^2 + Z_y^2) and phi = arctan2(Z_y, Z_x).

        # Displacement vectors (these are 2D vectors)
        d_intra = model.coords[1] - model.coords[0]
        d_inter = (model.coords[0] + model.lattice_constant) - model.coords[1]

        H_intra_p, H_inter_p = {}, {}

        # Calculate generalized Bessel arguments for INTRA-cell hopping
        Z_x_intra = (E_field / omega) * d_intra[0]
        Z_y_intra = chirality * (E_field / omega) * d_intra[1]
        Z_mag_intra = np.sqrt(Z_x_intra**2 + Z_y_intra**2)
        phi_intra = np.arctan2(Z_y_intra, Z_x_intra)

        # Calculate generalized Bessel arguments for INTER-cell hopping
        Z_x_inter = (E_field / omega) * d_inter[0]
        Z_y_inter = chirality * (E_field / omega) * d_inter[1]
        Z_mag_inter = np.sqrt(Z_x_inter**2 + Z_y_inter**2)
        phi_inter = np.arctan2(Z_y_inter, Z_x_inter)

        for p in all_p:
            # Intra-cell Fourier components
            phase_factor_intra = np.exp(-1j * p * phi_intra)
            bessel_val_intra = jv(p, Z_mag_intra)
            H_intra_p[p] = H_intra_static * bessel_val_intra * phase_factor_intra

            # Inter-cell Fourier components
            phase_factor_inter = np.exp(-1j * p * phi_inter)
            bessel_val_inter = jv(p, Z_mag_inter)
            H_inter_p[p] = H_inter_static * bessel_val_inter * phase_factor_inter
        # --- END OF MODIFICATION ---
        ...
        cbar = fig.colorbar(scatter, ax=ax)
        cbar.set_label('Mean Position in Chain ($\\langle x \\rangle$)', fontsize=12)
        ax.set_xlabel("State Index", fontsize=12)
        ax.set_ylabel("Quasi-energy", fontsize=12)
        ax.set_title(f"Floquet Spectrum at E = {E_field:.3f} (N={OBC_cells}, $\\eta$={chirality})", fontsize=14)
        ax.grid(True, linestyle='--', alpha=0.6)
        ax.set_ylim(-omega / 2, omega / 2)
        ax.set_xlim(OBC_cells * norbs * (nt/2 - 0.5), OBC_cells * norbs * (nt/2 + 0.5))
        ...
        for i, state_idx in enumerate(edge_state_indices):
            ...
            ax_edge.plot(np.arange(OBC_cells), prob_dist, 'o-', color='crimson', markersize=4)
            ax_edge.set_xlabel("Unit Cell Index", fontsize=12)
            ax_edge.set_ylabel("Probability Density $|\\psi(x)|^2$", fontsize=12)
            ax_edge.set_title(f"Distribution of Edge State {i+1} at E = {E_field:.3f} ($\\eta$={chirality})", fontsize=14)
            ax_edge.legend()
            ax_edge.set_xlim(-1, OBC_cells)
    ...
    print("--- Floquet OBC Analysis Finished ---")


if __name__ == "__main__":
    # main(omega=0.2)
    # main(omega=0.3)
    main(E_min = 0, E_max=0.5, E_step=201, omega=0.025, OBC_cells=20)
    # main(E_min = 0, E_max=0.5, E_step=201, omega=0.026)
    # main(E_min = 0, E_max=0.5, E_step=201, omega=0.027)
    # main(E_min = 0, E_max=0.5, E_step=201, omega=0.028)
    # main(E_min = 0, E_max=0.5, E_step=201, omega=0.029)
    # main(E_min = 0, E_max=0.5, E_step=201, omega=0.030)
    # main(E_min = 0, E_max=0.2, E_step=51, omega=0.25)
    # main(E_min = 0, E_max=0.2, E_step=51, omega=0.05)
    # main(E_max=0.05, E_step=51, omega=0.002, nt=201)
    # Run with Left-Circularly Polarized (LCP) light
    # main_quasi_2D(E_min=100,E_max=150, E_step=51, omega=10, chirality=1.0)

    # Run with Right-Circularly Polarized (RCP) light
    # main_quasi_2D(E_max=5, E_step=51, omega=2.5, chirality=-1.0)
    
    # Run with Linear light (original behavior)
    # main_quasi_2D(E_max=5, E_step=51, omega=2.5, chirality=0.0)