...
def scf_energy_from_mol(mol, tol=1e-8, max_cycle=100, damping=0.2, verbose=True):
    # Core pieces from Molecule.build()
    H = getattr(mol, 'hcore', getattr(mol, 'core', None))
    if H is None:
        raise ValueError("Molecule has no hcore/core. Call mol.build() first.")
    eri = getattr(mol, 'eri', None)
    if eri is None:
        raise ValueError("Molecule has no eri. Call mol.build() first.")

    nz, norb = mol.nz, mol.norb
    nao = H.shape[0]

    # Overlap S (if available), else assume orthonormal AO
    ...
    try:
        ...
        if not isinstance(charge, (int, float, np.integer, np.floating)):
            charge = 0
    ...
    # Orthonormalize AO space and make initial guess
    X = _orthogonalizer(S)
    Hp = X.T @ H @ X
    eps, Cprime = np.linalg.eigh(Hp)
    D = _make_density_from_XC(X, Cprime, nelec)
